...
function getQueryTerms(question) {
  ...
  return (normalizeSearchText(question).match(/[a-z0-9]{3,}/g) || [])
...
function rowMatchesQuery(row, terms) {
  ...
  return terms.some((term) => {
    if (term === "ingresos") return Number(row.ImporteNum || 0) >= 0;
    if (term === "egresos") return Number(row.ImporteNum || 0) < 0;
...
function renderMovementsTable() {
  ...
  if (state.queryFilterTerms.length) {
    activeColumnFilters.push(`Consulta: "${state.queryFilterTerms.join(" ")}"`);
  }
...
async function askAboutMovements() {
  ...
  try {
    ...
    const candidateRows = movementRows.filter((row) => {
      return rowMatchesQuery(row, terms);
    });
